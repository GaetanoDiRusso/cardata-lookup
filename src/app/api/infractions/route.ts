...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create logger
    const logger = new Logger(userContext.userId, 'infractions-api-route');
    logger.info('Starting infractions data retrieval via API route', { folderId });
    ...
    logger.info('Infractions data retrieval completed successfully');

    // Return the result
    return NextResponse.json(result.toPresentation(), { status: 200 });

  } catch (error: any) {
    console.error('Error in infractions API route:', error);
