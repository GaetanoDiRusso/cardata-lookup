...
export class MatriculaVehicleDataRetrievalUseCase extends BaseVehicleDataRetrievalUseCase {
  ...
  protected async callDataRetrievalService(
    ...
    logger.info('Calling getMatriculaRequeridaData service', {
      vehicleData,
      userId,
    });

    // Call the matricula service - it will throw on error
    const result = await dataRetrievalService.getMatriculaRequeridaData(userId, vehicleData, logger);

    // Include the logs from the service
    logger.addLogs(result.logs);

    if (result.success) {
      logger.info('getMatriculaRequeridaData service succeeded');
    } else {
      logger.error('getMatriculaRequeridaData service failed', {
        error: result.error,
      });
    }
